...
const { useState: useCourseState } = React;

function CoursePage({ course, onOpen, setRoute }) {
  // 本地章节完成态（驱动进度实时变化 + 顺序解锁）
  const [chapters, setChapters] = useCourseState(() => course.chapters.map(c => ({ ...c })));
  const firstUndone = Math.max(0, chapters.findIndex(c => !c.done));
  const [active, setActive] = useCourseState(firstUndone);
  const [tab, setTab] = useCourseState("catalog");
  const [bookmarked, setBookmarked] = useCourseState(false);
  const [celebrate, setCelebrate] = useCourseState(false);
