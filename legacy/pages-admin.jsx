...
function AdminCourses() {
  const [view, setView] = useAdminState(null);  // null=list | course对象 | "new"
...
function AdminTasks() {
  const [toastNode, toast] = useToast();
  const [courseId, setCourseId] = useAdminState(COURSES[1].id);
  const [target, setTarget] = useAdminState("sales");
  const [due, setDue] = useAdminState("2026-06-15");
  const [required, setRequired] = useAdminState(true);
  const [assignments, setAssignments] = useAdminState([
...
Object.assign(window, { AdminDash, AdminCourses, AdminTasks, AdminPeople });
