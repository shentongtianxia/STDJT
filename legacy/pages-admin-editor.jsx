...
function CourseEditor({ course, onBack, onToast }) {
  const isNew = !course;
  const [tab, setTab] = useEditorState("basic");
  const [title, setTitle] = useEditorState(course?.title || "");
  const [cat, setCat] = useEditorState(course?.cat || "onboard");
  const [instructor, setInstructor] = useEditorState(course?.instructor || "");
  const [desc, setDesc] = useEditorState(course?.desc || "");
  const [required, setRequired] = useEditorState(course?.required || false);
  const [chapters, setChapters] = useEditorState(() =>
    (course?.chapters || []).map((c, i) => ({ id: "ch" + i, t: c.t, type: c.type, d: c.d })));
  const [questions, setQuestions] = useEditorState(() =>
...
Object.assign(window, { CourseEditor, AField, AInput, ATextarea, ASelect });
