/* 神通大讲堂 — 考试中心（列表 + 答题 + 结果） */
const { useState: useExamState } = React;

function ExamPage() {
  const [taking, setTaking] = useExamState(null);
...
function ExamRunner({ exam, onExit }) {
  ...
  const [idx, setIdx] = useExamState(0);
  const [answers, setAnswers] = useExamState({});
  const [submitted, setSubmitted] = useExamState(false);
