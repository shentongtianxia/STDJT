...
/* ---------------- Avatar ---------------- */
export const AV_COLORS = ["#3A5A86","#3F6E60","#7C6242","#4E4D80","#3A6470","#5E5566"];
...
/* ---------------- Cover ---------------- */
export function Cover({ course, showPlay = true }) {
...
export const getNav = (mode) => (mode === "admin" ? ADMIN_NAV : LEARNER_NAV);
...
/* ---------------- Sidebar ---------------- */
export function Sidebar({ route, setRoute, mode, open, onClose }) {
...
/* ---------------- TopBar ---------------- */
export function TopBar({ crumb, onMenu, onSearch, searchValue, setSearchValue, onNav, onSettings, onLogout }) {
...
export function BottomNav({ route, setRoute, mode }) {
  const nav = getNav(mode);
